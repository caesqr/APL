...
=== OFFICIAL GUIDE ===
All code interpreted by msl.exe follow this format: <non-alphanumerical symbol(s)><operand(s)> (spaceless unless between two operands of the same symbol)
...
Below is the structure of the code inside "compile.txt". There are 4 sections: DLL imports, initialized data, uninitialized data and code. A section ends when a symbol from the next section is first read by the compiler. Thus, the compiler will not misinterpretate the same symbol across different sections. All symbols inside the same section can appear in any order.
